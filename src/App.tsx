...
export default function RetirementCalculator() {
  ...
  return (
    <div className="w-full max-w-6xl p-4 bg-white rounded-lg shadow space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Retirement Calculator</h2>
        <label className="flex items-center text-sm">
          <input type="checkbox" checked={showNominal} onChange={(e) => setShowNominal(e.target.checked)} className="mr-2"/>
          Show Future Dollars
        </label>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={formatNumber(desiredRetirementIncome)} 
          ...
          <div>
            ...
            <input type="text" value={formatNumber(currentSavings)}
          ...
          <div>
            ...
            <input type="text" value={formatNumber(monthlyContribution)}
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="number" value={currentAge} min="18" max={retirementAge}
              onChange={(e) => setCurrentAge(Number(e.target.value))}
              className="w-full p-2 border rounded"/>
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Retirement Age</label>
            <input type="number" value={retirementAge} min={currentAge} max={lifeExpectancy}
              onChange={(e) => setRetirementAge(Number(e.target.value))}
              className="w-full p-2 border rounded"/>
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Life Expectancy</label>
            <input type="number" value={lifeExpectancy} min={retirementAge} max={120}
        ...
        <div className="space-y-3">
          {Object.entries(ALLOCATIONS).map(([key, value]) => (
            <div key={key} className="flex items-center space-x-2">
              <input type="radio" id={key} checked={allocation === key}
                onChange={() => setAllocation(key)} className="form-radio"/>
      ...
      <div className="h-96">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={monteCarloData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
            ...
            <Tooltip formatter={(value) => [`$${formatNumber(value)}`, "Portfolio Value"]} labelFormatter={(age) => `Age: ${age}`}/>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={monteCarloData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
            ...
            <Tooltip formatter={(value, name) => {
              ...
              return [`${formatYears(value)} years`, 'Years of Income Coverage'];
            }}/>
